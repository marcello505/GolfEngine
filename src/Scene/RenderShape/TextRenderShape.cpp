...
namespace GolfEngine::Scene::Render {
    TextRenderShape::TextRenderShape(const Vector2 &pos, float rotation, std::string text, size_t fontSize,
                                     const Color &color, std::string filePath) :
            _position{pos}, _initialPosition(pos), _rotation{rotation}, _text{std::move(text)}, _fontSize{fontSize}, _color{color},
