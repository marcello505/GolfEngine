...
namespace GolfEngine::Scene::Render {
    SpriteRenderShape::SpriteRenderShape(std::string path, Vector2 position, float rotation, Vector2 pixelScale,
                                         Rect2 imageSource, Vector2 pivotPoint, Color color)
            : _position{position}, _initialPosition{position}, _path{std::move(path)}, _imageSource{imageSource},
              _rotation{rotation}, _color{color},
