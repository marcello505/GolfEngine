...
namespace GolfEngine::Scene::UI {
    ...
    Render::RenderShape &Button::getRenderShape() {
    ...
    void Button::setShape() {
        ...
        auto *rectRenderShape = new Render::RectRenderShape(Rect2(_position, Vector2(_width, _height)),
                                                    0,
                                                    Vector2(0, 0),
                                                    Color(255, 255, 255, 255));

        auto *textRenderShape = new Render::TextRenderShape(_text.position, _text.rotation, _text.value,
                                                    _text.fontSize, _text.color, _text.filePath);

        // setup render shape
        _buttonRenderShape = std::make_unique<Render::ButtonRenderShape>(rectRenderShape, textRenderShape,
                                                                 _text.alignment);
    ...
    void Button::onUpdate() {
        ...
        if (mousePos.x >= buttonXRange.first && mousePos.x <= buttonXRange.second &&
            ...
            if (ActionMap::getActionMap()->isJustPressed(
                    _onClickActionName)) { // if ClickButton action is just pressed,
