...
namespace GolfEngine::Scene {
    Render::RenderShape &Graph::getRenderShape() {
        return _renderShape;
    }

    Graph::Graph(std::vector<Node> nodes, std::vector<std::unique_ptr<Render::RectRenderShape>> drawables) : nodes{
            std::move(nodes)} {
        ...
        if (GolfEngine::Services::Render::hasService()) {
            ...
            if (settings.hasBool(PROJECT_SETTINGS_BOOL_RENDER_PATHFINDING) &&
                settings.getBool(PROJECT_SETTINGS_BOOL_RENDER_PATHFINDING)) {
