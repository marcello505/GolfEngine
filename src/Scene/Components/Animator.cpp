...
namespace GolfEngine::Scene::Components {
    Animator::Animator(std::string spriteSheetPath, int spriteSheetRows, int spriteSheetCols,
                       Vector2 spriteSheetCellSize, Vector2 pixelScale)
            : _spriteSheetPath{std::move(spriteSheetPath)}, _rows{spriteSheetRows}, _cols{spriteSheetCols},
              _cellSize{spriteSheetCellSize}, _pixelScale{pixelScale}, _fps{60},
    ...
    void Animator::onStart() {
        ...
        if (_gameObject->get().hasComponent<SpriteComponent>())
            ...
            _spriteComponent = _gameObject->get().addComponent<SpriteComponent>(_spriteSheetPath, _pixelScale,
                                                                                Rect2(Vector2(0, 0), _cellSize));
    }
        ...
        if (_currentAnimation != std::nullopt) {
            ...
            if (_countedFrames > _fps / _currentAnimation->get().playSpeed) {
                ...
                _spriteComponent->get().setImageSource(Rect2(Vector2{(_currentCell % _cols) * _cellSize.x,
                                                                     (int) floorf(
                                                                             (float) _currentCell / (float) _cols) *
                                                                     _cellSize.y},
                                                             _cellSize));
            }
        }
