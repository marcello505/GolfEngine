...
namespace GolfEngine::Scene::Components {
    Pathfinding::Pathfinding(GameObject *target, float recalculatePathTime) : _target{*target}, _fps{60},
                                                                              _recalculatePathTime{
                                                                                      recalculatePathTime} {
    }
    ...
    Vector2 Pathfinding::getDirection(Vector2 target) {
        ...
        } else if (target.x < getParentGameObjectPosition().x &&
                   target.y > getParentGameObjectPosition().y) { // leftDown
            newDir = Vector2(-1.0f, 1.0f);
        } else if (target.x > getParentGameObjectPosition().x &&
                   target.y < getParentGameObjectPosition().y) { // rightUp
            newDir = Vector2(1.0f, -1.0f);
        } else if (target.x > getParentGameObjectPosition().x &&
                   target.y > getParentGameObjectPosition().y) { // rightDown
            newDir = Vector2(1.0f, 1.0f);
        }
