...
namespace GolfEngine::Services::Render {
    ...
    public:
        ...
        [[nodiscard]] virtual std::optional<std::reference_wrapper<Scene::Camera>> getMainCamera() const = 0;

        /// Registers a camera to be used in the scene
        /// \param camera reference to camera, can be null to clear any camera
        virtual void setMainCamera(const std::optional<std::reference_wrapper<Scene::Camera>> &camera) = 0;

        /// Gets the offset created by the currently registered camera
        /// \return Vector2 of main camera offset
        [[nodiscard]] virtual Scene::Vector2 getCameraOffset() const = 0;
